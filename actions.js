...
// Set quick find input field as focus Setup Home
	$(document).arrive('.filter-box.input', function(){
		var $omniQuickFind = $(this);
		//console.log('filter found');
		setTimeout(
		  function() 
		  { $omniQuickFind.focus();
		  }, 1500);
		//console.log("globalMinify " + globalMinify);
		console.log("globalQuickFocus " + globalQuickFocus);
	});
...
// Set quick find input field as focus Setup > Object Manager
	$(document).arrive("#globalQuickfind", function() {
    	...
    	if(globalQuickFocus == true){
	    	...
	    	$('.slds-page-header').css("padding","0.3rem 1rem");
...
//*** Custom formatting ***//
  // Move Quick find filter to the left where all the other quick find filters are
	$(document).arrive("#globalQuickfind", function() {
    	// 'this' refers to the newly available element
  ...
  // Minimize the sandbox label header size
...
//	$(document).click(function(e) {
	  	...
	  	//Let's the page update
	  	//console.log('fired1');
	  	//$('.panelContainer').addClass('fired2');
